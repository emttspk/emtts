...
- 2026-06-07: Added a dedicated Google auth callback route for mobile/touch browsers, centralized redirect handling, and cleaned up login/register Google flows so successful signup/login exchanges the Firebase token, stores the session, and returns to the dashboard path. Validated with `npm.cmd run build` and `npm.cmd run auth:hammer`. See `docs/operations/auth-mobile-google-audit-2026-06-07.md`.
