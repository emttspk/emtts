...
- Several high-value acquisition and checkout-intent Meta events are still missing and only covered by GA4/internal analytics.
...
No Meta Pixel events were found in the helper that are defined but never reachable from code. The current gap is the opposite: important user actions are tracked in GA4/internal analytics but do not emit a Meta event at all.
...
| --- | --- | --- | --- | --- |
| `Lead` | Standard | Register / Start Free CTA flow (`lead_start` currently internal only) | `lead_start` | `Lead` |
| `ViewContent` | Standard | Pricing page / package cards / billing plan preview | `package_select` / `payment_start` | `ViewContent` |
| `InitiateCheckout` | Standard | Package select / payment start flow (`package_select`, `payment_start` currently internal only) | `package_select`, `payment_start` | `InitiateCheckout` |
...
| Package selection / payment intent | `InitiateCheckout` | Missing |
