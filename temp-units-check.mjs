...
const email = `railway.units.${Date.now()}@example.com`;
const password = 'SmokePass123!';
const trackingId = `VPL26${String(Math.floor(Math.random()*900000)+100000)}`;
const csvPath = path.join(os.tmpdir(), `railway-units-${Date.now()}.csv`);
const csv = [
  ...
  `Acme Store,03001234567,1 Mall Road,ops@acme.example,Lahore,Ali Raza,ali@example.com,03111222333,House 10 Street 5,Lahore,20000,UNITS-${Date.now()},Books,1.0,VPL,1,${trackingId}`
].join('\n');
...
try{
  await fetch(`${API}/api/auth/register`, {method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({email,password,companyName:'Units Co',address:'1 Mall Road',contactNumber:'03001234567',originCity:'Lahore'})});
  ...
  const before = Number(meBefore?.usage?.remainingUnits ?? meBefore?.usage?.unitsRemaining ?? meBefore?.unitsRemaining ?? -1);
  ...
  for(let i=0;i<120;i++){
    ...
    const b = await j(r);
    const st = String(b?.job?.status||'').toUpperCase();
    if(st==='COMPLETED') break;
    if(st==='FAILED') throw new Error(`job failed ${b?.job?.error||''}`);
    await sleep(2000);
  }

  const meAfterRes = await fetch(`${API}/api/me`, {headers:{authorization:`Bearer ${token}`}});
  const meAfter = await j(meAfterRes);
  const after = Number(meAfter?.usage?.remainingUnits ?? meAfter?.usage?.unitsRemaining ?? meAfter?.unitsRemaining ?? -1);

  console.log('UNITS_CHECK_JSON=' + JSON.stringify({email, jobId, before, after, deducted: before - after}));
}catch(e){
  console.error('UNITS_CHECK_FAILED', e?.message||e);
  process.exitCode=1;
}finally{
