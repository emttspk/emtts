 ...
 * operation fails.
 */
export async function assertWithinMonthlyLimit(userId, count) {
    ...
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            if (totalUsed > limit) {
                ...
                throw new Error(`Adding ${count} labels would exceed your monthly limit of ${limit}. You have used ${usage.labelsGenerated + usage.labelsQueued - count} labels.`);
            }
            return limitOk();
        });
    }
...
export async function assertWithinTrackingLimit(userId, count) {
    ...
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            const totalUsed = usage.labelsGenerated + usage.labelsQueued;
            if (totalUsed > limit) {
                throw new Error(`Adding ${count} tracking records would exceed your monthly unit limit of ${limit}. You have used ${usage.labelsGenerated + usage.labelsQueued - count} units.`);
            }
            return limitOk();
        });
    }
 ...
 */
export async function finalizeQueuedToGenerated(userId, count) {
    const month = new Date().toISOString().slice(0, 7);
...
export async function finalizeQueuedTrackingToGenerated(userId, count) {
    const month = new Date().toISOString().slice(0, 7);
    await prisma.usageMonthly.updateMany({
 ...
 */
export async function releaseQueuedLabels(userId, count) {
    const month = new Date().toISOString().slice(0, 7);
    await prisma.usageMonthly.updateMany({
        where: { userId, month, labelsQueued: { gte: count } },
        data: {
...
export async function releaseQueuedTracking(userId, count) {
    const month = new Date().toISOString().slice(0, 7);
    await prisma.usageMonthly.updateMany({
        where: { userId, month, trackingQueued: { gte: count } },
        data: {
