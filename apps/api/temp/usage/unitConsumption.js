...
async function ensureUsageLogsTable() {
  ...
  `);
    await prisma.$executeRawUnsafe("CREATE UNIQUE INDEX IF NOT EXISTS usage_logs_unique_request ON usage_logs(user_id, action_type, request_key)");
    usageLogsReady = true;
}
export async function consumeUnit(userId, actionType, requestKey) {
    return consumeUnits(userId, [{ actionType, requestKey }]);
}
export async function consumeUnits(userId, requests) {
    if (requests.length === 0)
        return { ok: true, remainingUnits: 0, idempotent: true };
    ...
    try {
        return await prisma.$transaction(async (tx) => {
            ...
            const existing = await tx.$queryRawUnsafe(`SELECT action_type, request_key, status FROM usage_logs WHERE user_id = ?`, userId);
            const existingMap = new Map(existing.map((row) => [`${row.action_type}::${row.request_key}`, row.status]));
            const pendingRequests = requests.filter((r) => !existingMap.has(`${r.actionType}::${r.requestKey}`));
            ...
            if (pendingRequests.length === 0) {
                const usage = (await tx.usageMonthly.findUnique({ where: { userId_month: { userId, month } } })) ??
                    { labelsGenerated: 0, labelsQueued: 0 };
            ...
            for (const req of pendingRequests) {
                await tx.$executeRawUnsafe(`INSERT OR IGNORE INTO usage_logs (id, user_id, action_type, units_used, request_key, status) VALUES (?, ?, ?, 1, ?, 'CONSUMED')`, randomUUID(), userId, req.actionType, req.requestKey);
            }
...
export async function refundUnit(userId, actionType, requestKey) {
    await refundUnits(userId, [{ actionType, requestKey }]);
}
export async function refundUnits(userId, requests) {
    if (requests.length === 0)
        return;
    await ensureUsageLogsTable();
    const month = monthKeyUTC();
    await prisma.$transaction(async (tx) => {
        for (const req of requests) {
            await tx.$executeRawUnsafe(`UPDATE usage_logs SET status = 'REFUNDED', refunded_at = CURRENT_TIMESTAMP WHERE user_id = ? AND action_type = ? AND request_key = ? AND status = 'CONSUMED'`, userId, req.actionType, req.requestKey);
        }
...
export async function refundUnitsByAmount(userId, units) {
    if (units <= 0)
        return;
    const month = monthKeyUTC();
