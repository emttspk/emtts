...
function extractTrackingSteps(rawData) {
...
function extractTrackingEvents(rawData) {
    if (!rawData || typeof rawData !== "object")
        return [];
    const raw = rawData;
    const tracking = (raw.tracking && typeof raw.tracking === "object" ? raw.tracking : raw);
...
function parseSystemMoTokens(explicitMo) {
...
function cityToken(input) {
    const words = String(input ?? "").toUpperCase().match(/[A-Z]+/g) ?? [];
    return words[0] ?? "";
}
function inactivityHoursFromEvents(events) {
...
function resolveDeliveryOfficeFromHistory(events, fallbackDeliveryOffice) {
    const cleanOffice = (value) => {
        return String(value ?? "")
            .replace(/\(bagid:.*?\)/gi, "")
    ...
    const extractOfficeFromDetail = (detailRaw) => {
...
function extractSelectedSectionMosId(rawData) {
    if (!rawData || typeof rawData !== "object")
        return null;
    const top = rawData;
...
function extractFullPageMosId(rawData) {
    if (!rawData || typeof rawData !== "object")
        return null;
    const top = rawData;
    const tracking = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
...
function parseCollectedAmount(rawData) {
    if (!rawData || typeof rawData !== "object")
        return 0;
    const top = rawData;
    const tracking = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
...
function hasCollectedAmountField(rawData) {
    if (!rawData || typeof rawData !== "object")
        return false;
    const top = rawData;
    const tracking = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
...
function parseServiceType(rawData, trackingNumber) {
...
function deriveSystemStatus(input) {
    ...
    const has = (needle) => normalizedEvents.some((e) => e.text.includes(needle));
    const hasAny = (needles) => normalizedEvents.some((e) => needles.some((n) => e.text.includes(n)));
    ...
    if (codFamily && hasMosUpdated) {
        const deliveredAtBookingOffice = normalizedEvents.some((ev) => {
            if (!ev.text.includes("delivered at delivery office") && !ev.text.includes("delivered to addressee"))
                return false;
        ...
        if (hasOutForDelivery && movedBackToBooking)
            return "PENDING";
        if (movedBackToBooking || reverseDispatchToBooking)
            return "PENDING";
        return "PENDING";
    }
    ...
    if (codFamily) {
        ...
        for (let i = 0; i < normalizedEvents.length; i += 1) {
            ...
            if ((t.includes("delivered to addressee") || t.includes("delivered at delivery office")) &&
        ...
        if (deliveredIdx >= 0) {
            ...
            if (noFurtherTracking || movedBackToBooking) {
                if ((inactivityHours ?? 0) >= 72)
                    return "PENDING";
                if ((inactivityHours ?? 0) >= 48)
                    return "PENDING";
                if ((inactivityHours ?? 0) >= 24)
                    return "PENDING";
                return "PENDING";
            }
        }
    ...
    const deliveredAtBookingOffice = normalizedEvents.some((ev) => {
        if (!ev.text.includes("delivered"))
            return false;
        if (ev.text.includes("undelivered"))
            return false;
        if (ev.text.includes("to sender"))
            return true;
        if (bookingToken && ev.upper.includes(bookingToken))
            return true;
        return ev.text.includes("booking office");
    });
    if (deliveredToSender || deliveredAtBookingOffice)
        return "RETURNED";
    if (movedBackToBooking || reverseDispatchToBooking)
    ...
    if (normalizedEvents.some((e) => {
        if (!e.text.includes("dispatch from dmo"))
            return false;
        if (bookingToken && !e.upper.includes(bookingToken))
            return false;
        if (e.text.includes("to dmo"))
            return true;
        return false;
    })) {
        return "PENDING";
    ...
    const arrivedAtDeliveryCity = normalizedEvents.some((e) => {
        if (e.text.includes("dispatch to delivery office"))
            return true;
        if (!e.text.includes("delivery office"))
            return false;
        return deliveryToken ? e.upper.includes(deliveryToken) : true;
    });
    if (arrivedAtDeliveryCity && hasOutForDelivery)
        return "PENDING";
    if (arrivedAtDeliveryCity)
        return "PENDING";
    ...
    if (latestEvent?.dt) {
        ...
        const reachedDeliveryCity = normalizedEvents.some((e) => {
            if (deliveryToken && e.upper.includes(deliveryToken))
                return true;
            return e.text.includes("delivery office");
        });
        if (latestText.includes("delivery office") && ageHours >= 72)
            return "PENDING";
        if (ageHours >= 72)
            return "PENDING";
        if (reachedDeliveryCity && ageHours >= 48)
            return "PENDING";
        if (ageHours >= 24 && (arrivedAtDeliveryCity || hasOutForDelivery))
            return "PENDING";
    }
    ...
    if (hasAny(["dispatch", "received at", "arrival"]))
        return "PENDING";
    return "ACTIVE";
}
function deriveTrackingCategory(systemStatus) {
    const s = String(systemStatus ?? "").trim().toUpperCase();
    if (s === "DELIVERED" || s === "DELIVERED WITH PAYMENT")
        return "DELIVERED_COMPLETE";
    if (s === "RETURN")
        return "RETURN_ACTION_REQUIRED";
    if (s === "FAILED_DELIVERY")
        return "FAILED_ATTEMPT";
    if (s === "RETURN_IN_PROCESS" || s === "RETURN_INITIATED" || s === "RETURNED")
        return "RETURN_ACTION_REQUIRED";
    if (s === "PENDING")
        return "ACTIVE";
...
export function getFinalStatus(trackingSteps) {
...
export function processTracking(rawData, opts) {
    const steps = extractTrackingSteps(rawData);
    const events = extractTrackingEvents(rawData);
    const top = (rawData && typeof rawData === "object" ? rawData : {});
    ...
    let normalizedStepStatus = "UNKNOWN";
    for (const step of steps) {
        const s = normalizeStepText(step);
        if (s.includes("delivered to addressee") || s.includes("delivered at delivery office")) {
        ...
        if (s.includes("received at dmo")) {
            if (normalizedStepStatus === "UNKNOWN")
                normalizedStepStatus = "BOOKED";
        }
    }
    const legacyFinalStatus = getFinalStatus(steps);
    const moMatched = trackingMo !== "-" &&
    ...
    const isMosStep = (s) => s.includes("mos") || s.includes("money order");
    const isArticleStep = (s) => !isMosStep(s);
    const articleDelivered = normalizedEvents.some((s) => isArticleStep(s) &&
        (s.includes("delivered to addressee") || s.includes("delivered at delivery office")) &&
        !s.includes("undelivered"));
    const articleUndelivered = normalizedEvents.some((s) => isArticleStep(s) && (s.includes("undelivered") || s.includes("deposit") || s.includes("refused")));
    ...
    const activeDeliveryPhaseLast = lastEventText.includes("delivery office") ||
        lastEventText.includes("sent out for delivery") ||
        lastEventText.includes("dispatch from") ||
        lastEventText.includes("dispatch to");
    const topLevel = (rawData && typeof rawData === "object" ? rawData : {});
    ...
    if (steps.length > 0) {
        ...
        if (normalizedSystem === "DELIVERED" || normalizedSystem === "DELIVERED WITH PAYMENT")
            finalStatus = "DELIVERED";
        else if (normalizedSystem === "RETURN" || normalizedSystem === "RETURNED")
            finalStatus = "RETURN";
        else
            finalStatus = "PENDING";
    }
    ...
    const daysPassed = firstDate && !Number.isNaN(firstDate.getTime())
        ...
        : null;
    const manualPendingOverride = Boolean(trackingNode.manual_pending_override ?? topLevel.manual_pending_override);
    const mosNumber = (() => {
        if (tnStartsWithMos(opts?.trackingNumber))
            return String(opts?.trackingNumber).trim().toUpperCase();
        if (sectionMos)
            return sectionMos;
        if (fullPageMos)
            return fullPageMos;
    ...
    const fullTrackingCapturedFlag = Boolean(trackingNode.full_tracking_captured ?? topLevel.full_tracking_captured);
...
function tnStartsWithMos(trackingNumber) {
    return String(trackingNumber ?? "").trim().toUpperCase().startsWith("MOS");
}
export function canonicalShipmentStatus(status, moIssued) {
    const raw = String(status ?? "").trim().toUpperCase();
    if (!raw || raw === "-")
        return "PENDING";
    if (raw === "DELIVERED")
        return "DELIVERED";
    if (raw === "DELIVERED WITH PAYMENT")
        return "DELIVERED";
    if (raw === "RETURN" || raw === "RETURNED" || raw === "FAILED_DELIVERY")
        return "RETURN";
    if (raw === "RETURN_IN_PROCESS" || raw === "RETURN_INITIATED")
        return "PENDING";
    if (raw === "PENDING")
        return "PENDING";
    ...
    if (raw.includes("DELIVER"))
        return "DELIVERED";
    if (raw.includes("RETURN") || raw.includes("RTO"))
        return "RETURN";
    return "PENDING";
}
export function trackingFound(status) {
 ...
 */
export function isComplaintEnabled(daysPassed, systemStatus, inactivityHours) {
 ...
 */
export function calculateStatusCards(shipments) {
    let delivered = 0;
    let pending = 0;
    let returned = 0;
    let delayed = 0;
