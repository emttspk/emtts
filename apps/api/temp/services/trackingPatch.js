...
const ALLOWED_FINAL_STATUSES = new Set(["Pending", "Delivered", "Return"]);
function asText(v) {
...
function isDispatch(text) {
    return text.includes("dispatch");
}
function isDispatchFromOrigin(text) {
    return text.includes("dispatch from dmo") || text.includes("dispatched from dmo");
}
function isReturnSignal(text) {
    return (text.includes("undelivered") ||
        text.includes("refused") ||
        text.includes("return to sender") ||
        text.includes("returned to sender"));
}
function isReturnCompleted(text) {
    return (text.includes("return to sender") ||
        text.includes("returned to sender") ||
        text.includes("return completed"));
}
function isFalseDeliveredOfficeText(text) {
...
function isStrictDelivered(text) {
    if (text.includes("undelivered") || text.includes("return") || text.includes("refused"))
        return false;
    if (isFalseDeliveredOfficeText(text))
        return false;
    if (text === "delivered")
        return true;
    if (text.includes("delivered to addressee"))
        return true;
    if (text.includes("delivered") && (text.includes("to addressee") || text.includes("to addresse")))
        return true;
    return /\bdelivered\b[\s\S]*\bto addres{1,2}e?\b/i.test(text);
}
function extractMosInfo(events) {
    for (let i = events.length - 1; i >= 0; i -= 1) {
        const text = lower(events[i].description);
        if (!text.includes("mos issued"))
            continue;
...
function isArrival(text) {
    return text.includes("arrival") || text.includes("arrived at") || text.includes("received at");
}
function isDispatchToDeliveryOffice(text) {
    return text.includes("dispatch to delivery office");
}
function isArrivalAtDeliveryOffice(text) {
    return (text.includes("arrival at delivery office") ||
        text.includes("arrived at delivery office") ||
        text.includes("received at delivery office"));
}
...
function extractOriginTokens(events) {
...
function reachesOrigin(event, originTokens) {
...
function buildCycles(events, originTokens) {
    if (events.length === 0)
        return [];
    const starts = [0];
    ...
    for (let i = 0; i < events.length; i += 1) {
        ...
        if (returnSignal) {
            ...
            if (i > lastStart && !starts.includes(i))
                starts.push(i);
        }
        if (inReturnPhase && atOrigin)
            returnReachedOrigin = true;
...
function hasStrictDeliveryFlow(events) {
    let stage = 0;
    for (const ev of events) {
        ...
        if (stage >= 4 && isStrictDelivered(text))
            return true;
    }
    return false;
}
...
function isBookingDmoLocation(location, bookingOffice) {
    ...
    return Boolean(upperLocation.includes("DMO") &&
        normalizedLocation &&
        normalizedBooking &&
        normalizedLocation.includes(normalizedBooking));
}
function isReturnDeliveredAtOrigin(lastEvent, bookingOffice) {
    if (!lastEvent)
        return false;
    ...
    const explicitDelivered = description.includes("delivered to sender") ||
    ...
    if (explicitDelivered)
        return true;
    const receiptOnlyAtOrigin = (atBookingOffice || atBookingDmo) &&
        /(received|arrival|arrived)/.test(description) &&
        !description.includes("delivered");
    if (receiptOnlyAtOrigin)
        return false;
...
function shouldDowngradeReturnToPending(finalStatus, lastEvent, bookingOffice) {
    if (finalStatus !== "Return" || !lastEvent)
        return false;
    const description = lower(lastEvent.description);
    const atBookingOffice = sameOffice(lastEvent.location, bookingOffice);
    const atBookingDmo = isBookingDmoLocation(lastEvent.location, bookingOffice);
    ...
    const receiptOnlyAtOrigin = (atBookingOffice || atBookingDmo) &&
        /(received|arrival|arrived)/.test(description) &&
        !description.includes("delivered");
    const returnMovementOnly = description.includes("return to sender") ||
...
function deriveMeta(events, manualPendingOverride) {
    ...
    if (hasDeliveredSignal) {
        ...
        if (hasPostDeliveryMovement)
            hasDeliveredSignal = false;
    }
    ...
    const complaintEnabled = finalStatus === "Pending" &&
    ...
    const complaintRule = !complaintEnabled || finalStatus === "Pending";
    ...
    const firstLtLast = events.length < 2 ||
        !firstTs ||
        !lastTs ||
        firstTs.getTime() <= lastTs.getTime();
    const invalidStatusLabel = ["PENDING_72H", "ARRIVED_AT_DELIVERY_CITY", "IN_TRANSIT"].includes(finalStatus.toUpperCase());
...
export function applyTrackingPatchLayer(response, opts) {
    ...
    meta.final_status = normalizePatchedFinalStatus(response?.final_status ?? response.meta?.final_status ?? response.status);
    const sourceCycle = Number(response?.current_cycle ?? response.meta?.current_cycle ?? 0);
    ...
    const sourceCycleDescription = asText(response?.cycle_description ?? response.meta?.cycle_description);
    if (sourceCycleDescription)
        meta.cycle_description = sourceCycleDescription;
    const sourceDecisionReason = asText(response?.reason ?? response.meta?.decision_reason);
    if (sourceDecisionReason)
        meta.decision_reason = sourceDecisionReason;
    ...
    console.log(`[TRACE] PATCH_APPLIED = TRUE | status_before_patch=${statusBeforePatch} status_after_patch=${meta.final_status} event_count=${displayEvents.length} first_event_in=${firstIn} last_event_in=${lastIn} first_event_out=${firstEventDate} last_event_out=${lastEventDate} complaint_enabled=${meta.complaint_enabled} order_asc=${meta.audit.sorted}`);
    console.log(`[TRACE] CYCLE_AUDIT total_cycles=${meta.total_cycles} final_cycle=${meta.final_cycle_index} final_status=${meta.final_status} last_event="${meta.last_event}" reason="${meta.decision_reason}"`);
