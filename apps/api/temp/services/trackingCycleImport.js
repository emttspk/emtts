...
import { getTrackingCycleCorrections, saveTrackingCycleCorrections } from "./trackingCycleCorrections.js";
import { PrismaClient } from "@prisma/client";
const prisma = new PrismaClient();
export async function validateAndImportCycleAudit(csvRows) {
    const errors = [];
    const warnings = [];
    const validCorrections = [];
    const statusChanges = {};
    const cycleChanges = {};
    const issueCodePatterns = {};
    ...
    if (validCorrections.length > 0) {
        try {
            ...
            const correction_array = validCorrections.map((c) => ({
                ...
                cycle_detected: c.cycle_detected,
