...
function normalizeEventType(detail) {
    ...
    if (t.includes("return to sender") || t.includes("returned to sender"))
        return "RETURNED";
    if (t.includes("return"))
        return "RETURN_IN_PROCESS";
    if (t.includes("undelivered") || t.includes("refused") || t.includes("deposit") || t.includes("not found"))
        return "FAILED_DELIVERY";
    if (t.includes("delivered") && !t.includes("undelivered"))
        return "DELIVERED";
    if (t.includes("sent out for delivery") || t.includes("out for delivery"))
        return "OUT_FOR_DELIVERY";
    if (t.includes("arrived at delivery office") || t.includes("arrival at delivery office") || t.includes("received at delivery office"))
        return "ARRIVED_DELIVERY_OFFICE";
    if (t.includes("arrived at dmo") || t.includes("arrival at dmo"))
        return "ARRIVED_DMO";
    if (t.includes("dispatch from dmo") || t.includes("dispatch from district mail office") || t.includes("dispatched from dmo"))
        return "DISPATCH_DMO";
    if (t.includes("received at dmo"))
        return "RECEIVED_DMO";
    if (t.includes("booked at"))
        return "BOOKED";
    return "RECEIVED_DMO";
}
function normalizeOfficeType(detail) {
    const t = txt(detail).toLowerCase();
    if (t.includes("dmo") || t.includes("district mail office"))
        return "DMO";
    if (t.includes("delivery office"))
        return "DELIVERY_OFFICE";
    if (t.includes("booked at"))
        return "BOOKING";
    return "BOOKING";
}
function extractEvents(rawData, trackingId) {
    if (!rawData || typeof rawData !== "object")
        return [];
    const top = rawData;
    const tracking = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
...
function toServiceType(trackingId) {
...
function computeMetrics(events, systemStatus) {
    ...
    let delayFlag = "NO";
    if (typeof totalDeliveryTimeHours === "number" && totalDeliveryTimeHours >= 72)
        delayFlag = "YES";
    const s = txt(systemStatus).toUpperCase();
    if (["PENDING_72H", "CRITICAL_DELAY", "HELD_AT_RLO"].includes(s))
        delayFlag = "YES";
    return {
...
export async function persistTrackingIntelligence(input) {
    ...
    if (!userId || !trackingId)
        return;
    const top = (input.rawData && typeof input.rawData === "object" ? input.rawData : {});
    const trackingNode = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
    ...
    await prisma.$transaction(async (tx) => {
        await tx.$executeRawUnsafe(`INSERT INTO tracking_core_intelligence
       ...
       ON CONFLICT(id) DO UPDATE SET
         ...
         updated_at=CURRENT_TIMESTAMP`, coreId, userId, trackingId, serviceType, bookingDate, originCity, destinationCity, bookingOffice || "-", resolvedDeliveryOffice || "-", currentStatus || "-", systemStatus || "-", isDelivered ? 1 : 0, isReturned ? 1 : 0);
        await tx.$executeRawUnsafe(`DELETE FROM tracking_events_intelligence WHERE user_id = ? AND tracking_id = ?`, userId, trackingId);
        for (const ev of events) {
            await tx.$executeRawUnsafe(`INSERT INTO tracking_events_intelligence
         (id, user_id, tracking_id, event_time, event_city, event_description, event_type, office_type, bag_id, event_order)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, randomUUID(), userId, trackingId, ev.eventTime, ev.eventCity || "-", ev.eventDescription, ev.eventType, ev.officeType, null, ev.eventOrder);
        }
        await tx.$executeRawUnsafe(`INSERT INTO tracking_derived_metrics_intelligence
       ...
       ON CONFLICT(id) DO UPDATE SET
         ...
         updated_at=CURRENT_TIMESTAMP`, coreId, userId, trackingId, metrics.totalDeliveryTimeHours, metrics.dmoToDmoTime, metrics.lastMileTime, metrics.returnTime, metrics.deliveryAttempts, metrics.delayFlag);
        const collectAmount = num(top?.CollectAmount ?? trackingNode?.CollectAmount);
        const paymentStatus = systemStatus === "DELIVERED" && serviceType.startsWith("V")
            ? "PAYMENT_PROCESSING"
            : systemStatus.startsWith("RETURN")
                ? "PAYMENT_STOPPED"
                : "PENDING";
        await tx.$executeRawUnsafe(`INSERT INTO cod_payment_tracking_intelligence
       ...
       ON CONFLICT(id) DO UPDATE SET
         ...
         updated_at=CURRENT_TIMESTAMP`, coreId, userId, trackingId, collectAmount, paymentStatus, null, null);
    });
}
export async function refreshTrackingIntelligenceAggregates(userIdInput) {
    await ensureIntelligenceTables();
    const userId = txt(userIdInput);
    if (!userId)
        return;
    const core = await prisma.$queryRawUnsafe(`SELECT * FROM tracking_core_intelligence WHERE user_id = ?`, userId);
    const metrics = await prisma.$queryRawUnsafe(`SELECT * FROM tracking_derived_metrics_intelligence WHERE user_id = ?`, userId);
    const events = await prisma.$queryRawUnsafe(`SELECT * FROM tracking_events_intelligence WHERE user_id = ?`, userId);
    const metricByTracking = new Map();
    metrics.forEach((m) => metricByTracking.set(String(m.tracking_id), m));
    const routeMap = new Map();
    const officeMap = new Map();
    const timeMap = new Map();
    const returnCityMap = new Map();
    for (const c of core) {
        ...
        const route = routeMap.get(routeKey) ?? { originCity: origin, destinationCity: destination, total: 0, delivered: 0, returned: 0, delay: 0, deliveryTimes: [] };
        route.total += 1;
        if (Number(c.is_delivered) === 1)
            route.delivered += 1;
        if (Number(c.is_returned) === 1)
            route.returned += 1;
        if (txt(m.delay_flag).toUpperCase() === "YES")
            route.delay += 1;
        if (Number.isFinite(Number(m.total_delivery_time_hours)))
            route.deliveryTimes.push(Number(m.total_delivery_time_hours));
        routeMap.set(routeKey, route);
        const office = officeMap.get(officeKey) ?? { deliveryOffice, city: destination, total: 0, delivered: 0, returned: 0, failed: 0, lastMileTimes: [] };
        office.total += 1;
        if (Number(c.is_delivered) === 1)
            office.delivered += 1;
        if (Number(c.is_returned) === 1)
            office.returned += 1;
        if (Number.isFinite(Number(m.last_mile_time)))
            office.lastMileTimes.push(Number(m.last_mile_time));
    ...
    const byTrackingEvents = new Map();
    for (const e of events) {
    ...
    for (const c of core) {
        ...
        const bucket = returnCityMap.get(city) ?? { city, total: 0, returned: 0, returnDays: [], reasons: new Map() };
        bucket.total += 1;
        if (Number(c.is_returned) === 1) {
            bucket.returned += 1;
            const m = metricByTracking.get(String(c.tracking_id));
            if (Number.isFinite(Number(m?.return_time)))
                bucket.returnDays.push(Number(m.return_time) / 24);
    ...
    await prisma.$transaction(async (tx) => {
        ...
        for (const r of routeMap.values()) {
            ...
            await tx.$executeRawUnsafe(`INSERT INTO route_performance_intelligence
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, r.originCity, r.destinationCity, r.total, r.delivered, r.returned, avg(r.deliveryTimes), delayRate, successRate);
        }
        for (const o of officeMap.values()) {
            const failureRate = o.total > 0 ? (o.failed / o.total) * 100 : 0;
            await tx.$executeRawUnsafe(`INSERT INTO delivery_office_performance_intelligence
         (id, user_id, delivery_office, city, total_parcels, delivered_count, returned_count, avg_last_mile_time, failure_rate, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, o.deliveryOffice, o.city, o.total, o.delivered, o.returned, avg(o.lastMileTimes), failureRate);
        }
        ...
        for (const r of reasonMap.values()) {
            await tx.$executeRawUnsafe(`INSERT INTO failure_reasons_intelligence
         (id, user_id, reason, city, delivery_office, count, percentage, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, r.reason, r.city, r.deliveryOffice, r.count, (r.count / totalFailures) * 100);
        }
        for (const t of timeMap.values()) {
            const successRate = t.total > 0 ? (t.delivered / t.total) * 100 : 0;
            await tx.$executeRawUnsafe(`INSERT INTO time_analysis_intelligence
         (id, user_id, hour_of_day, day_of_week, delivery_count, success_rate, avg_delivery_time, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, t.hour, t.day, t.delivered, successRate, null);
        }
        for (const b of returnCityMap.values()) {
            ...
            await tx.$executeRawUnsafe(`INSERT INTO return_behavior_intelligence
         (id, user_id, city, return_rate, avg_return_days, common_reason, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, b.city, returnRate, avg(b.returnDays), commonReason);
        }
    });
    await refreshPredictionAndBusinessLayer(userId);
}
async function refreshPredictionAndBusinessLayer(userId) {
    const core = await prisma.$queryRawUnsafe(`SELECT * FROM tracking_core_intelligence WHERE user_id = ?`, userId);
    const metrics = await prisma.$queryRawUnsafe(`SELECT * FROM tracking_derived_metrics_intelligence WHERE user_id = ?`, userId);
    const routes = await prisma.$queryRawUnsafe(`SELECT * FROM route_performance_intelligence WHERE user_id = ?`, userId);
    const returnBehavior = await prisma.$queryRawUnsafe(`SELECT * FROM return_behavior_intelligence WHERE user_id = ?`, userId);
    const avg = (arr) => (arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : null);
    ...
    const sellerAvg = avg(metrics.map((m) => Number(m.total_delivery_time_hours)).filter((v) => Number.isFinite(v))) ?? 0;
    await prisma.$transaction(async (tx) => {
        ...
        for (const c of core) {
            ...
            await tx.$executeRawUnsafe(`INSERT INTO tracking_predictions_intelligence
         (id, user_id, tracking_id, expected_delivery_time_hours, delay_probability, return_probability, risk_band, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, randomUUID(), userId, txt(c.tracking_id), expectedDelivery, delayProb, returnProb, riskBand);
        ...
        await tx.$executeRawUnsafe(`INSERT INTO business_dashboard_intelligence
       ...
       ON CONFLICT(user_id) DO UPDATE SET
         ...
         updated_at=CURRENT_TIMESTAMP`, userId, userId, JSON.stringify(bestCities), JSON.stringify(worstRoutes), JSON.stringify(codRiskZones));
    });
}
