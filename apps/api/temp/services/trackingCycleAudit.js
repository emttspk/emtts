...
function normalizeExpected(value) {
    const raw = text(value).toUpperCase();
    if (raw === "DELIVERED" || raw === "DELIVERED WITH PAYMENT")
        return "DELIVERED";
    if (raw === "RETURNED")
        return "RETURNED";
    return "PENDING";
...
function extractMosSourceSignals(raw) {
    ...
    if (!mosDeliveredActual) {
        mosDeliveredActual = events.some((ev) => {
            ...
            return ((d.includes("delivered to sender") || (d.includes("mos") && d.includes("delivered")))
...
function issueCodeFrom(interpretation, mismatch) {
    ...
    if (interpretation.mos_status === "MISSING")
        return "COD_MOS_MISSING";
    if (interpretation.mos_status === "IN_PROGRESS")
        return "MOS_IN_PROCESS";
    if (interpretation.cycle_detected === "Cycle 2" && interpretation.cycle_status !== "COMPLETED")
        return "RETURN_CYCLE_INCOMPLETE";
    if (interpretation.cycle_detected === "Cycle 1" && interpretation.cycle_status !== "COMPLETED")
        return "DELIVERY_STUCK";
    if (mismatch)
        return "STATUS_MISMATCH";
    return "NO_ISSUE";
}
function buildReason(interpretation, mismatch) {
    if (interpretation.final_status === "DELIVERED WITH PAYMENT" || interpretation.mos_status === "COMPLETED") {
        return "MOS detected, linked, and payment cycle completed.";
    }
    if (interpretation.flags.includes("MOS_REDIRECT"))
        return "MOS detected and verified from linked secondary tracking block.";
    if (interpretation.flags.includes("MOS_DETECTED_FROM_RAW") && interpretation.mos_status === "MISSING")
        return "MOS detected in source data but not linked into cycle interpretation.";
    if (interpretation.flags.includes("MOS_MISSING"))
        return "COD/VPL/VPP delivery is incomplete because MOS leg is missing.";
    if (interpretation.flags.includes("MOS_IN_PROGRESS"))
        return "Article delivered. MOS (Money Order) issued and in transit back to sender — awaiting final delivery.";
    if (interpretation.flags.includes("MOS_DETECTED_FROM_RAW"))
        return "MOS number detected in stored metadata. MOS tracking cycle is active.";
    if (interpretation.flags.includes("OUT_OF_ORDER_INPUT"))
        return "Input contained out-of-order scans; chronology was normalized before interpretation.";
    if (interpretation.flags.includes("DUPLICATE_EVENTS_REMOVED"))
        return "Duplicate scans were detected and removed during interpretation.";
    if (interpretation.flags.includes("LATEST_CYCLE_ONLY"))
        return "Multiple loops detected; latest cycle was used by policy.";
    if (mismatch)
        return "Current saved status does not match interpreted cycle status.";
    return "Current status is aligned with interpreted tracking cycle.";
}
function allowedReason(currentNorm, interpretation) {
...
function suggestedFix(issueCode) {
    if (issueCode === "MOS_VERIFIED_COMPLETED")
        return "No correction required. MOS verified and completed.";
    if (issueCode === "COD_MOS_MISSING")
        return "Keep status Pending and wait for MOS issued/booked/dispatch/delivered events.";
    if (issueCode === "MOS_DETECTED_NOT_LINKED")
        return "Link detected MOS record to article flow and reprocess combined cycle.";
    if (issueCode === "MOS_IN_PROCESS")
        return "Article delivered. MOS payment in transit — keep Pending until MOS delivered event appears.";
    if (issueCode === "RETURN_CYCLE_INCOMPLETE")
        return "Verify return dispatch and booking-office completion scans before marking Returned.";
    if (issueCode === "DELIVERY_STUCK")
        return "Keep Pending/Hold until delivered or return-start scans appear.";
    if (issueCode === "STATUS_MISMATCH")
        return "Align final status with detected cycle and reprocess interpretation.";
    return "No correction required.";
}
function missingSignals(interpretation) {
    const out = [];
    if (interpretation.flags.includes("MOS_MISSING"))
        out.push("MOS issuance or MOS transit scans missing.");
    if (interpretation.flags.includes("MOS_IN_PROGRESS"))
        out.push("MOS issued — awaiting delivery to sender.");
    if (interpretation.flags.includes("MOS_REDIRECT"))
        out.push("MOS tracking confirmed via secondary article block.");
    if (interpretation.flags.includes("MOS_DETECTED_FROM_RAW"))
        out.push("MOS number detected in stored metadata fields.");
    if (interpretation.flags.includes("OUT_OF_ORDER_INPUT"))
        out.push("Chronology conflict detected in source events.");
    if (interpretation.flags.includes("DUPLICATE_EVENTS_REMOVED"))
        out.push("Duplicate scans were present in source history.");
    return out;
}
function applyCorrection(record, correction) {
    return record;
}
export function buildTrackingCycleAuditRecord(shipment, correctionLookup) {
    ...
    const informationalOnly = issueCode === "MOS_VERIFIED_COMPLETED" ||
