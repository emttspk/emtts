...
function isCodScope(trackingNumber, raw) {
    const tn = trackingNumber.toUpperCase();
    if (tn.startsWith("COD") || tn.startsWith("VPL") || tn.startsWith("VPP"))
        return true;
    if (!raw || typeof raw !== "object")
        return false;
    const top = raw;
    const nested = (top.tracking && typeof top.tracking === "object" ? top.tracking : top);
    const serviceType = text(nested.service_type ?? nested.shipmentType ?? nested.shipmenttype ?? top.service_type).toUpperCase();
    if (serviceType.includes("COD") || serviceType.includes("VPL") || serviceType.includes("VPP"))
        return true;
...
function normalizeEventType(description) {
    ...
    if (/\bmos\b/.test(t) && /issued/.test(t))
        return "MONEY_ORDER_ISSUED";
    if (/\bmos\b/.test(t) && /booked/.test(t))
        return "MOS_BOOKED";
    if (/\bmos\b/.test(t) && /dispatch|dispatched/.test(t))
        return "MOS_DISPATCH";
    if (/\bmos\b/.test(t) && /delivered/.test(t) && !/undelivered/.test(t))
        return "MOS_DELIVERED";
    ...
    if (t.includes("delivered to sender") && !t.includes("undelivered"))
        return "MOS_DELIVERED";
...
function computeLatestCycleStart(events) {
    if (events.length <= 1)
        return 0;
    let cycleStart = 0;
    for (let i = 0; i < events.length; i += 1) {
        ...
        const tailHasProgress = tail.some((event) => (event.normalized_type === "RECEIVED_AT_DELIVERY_OFFICE"
        ...
        const priorHasTerminalOrReturn = events.slice(0, i).some((event) => (event.normalized_type === "DELIVERED"
            || event.normalized_type === "MOS_DELIVERED"
            || event.normalized_type === "UNDELIVERED"
            || event.normalized_type === "RETURN_DISPATCH"));
        const tailHasAttemptOrDelivered = tail.some((event) => (event.normalized_type === "OUT_FOR_DELIVERY"
...
function detectState(cycleEvents) {
    ...
    for (let i = 0; i < cycleEvents.length; i += 1) {
        ...
        if (event.normalized_type === "BOOKED")
            state.hasBooked = true;
        if (event.normalized_type === "RECEIVED_AT_DMO")
            state.hasReceivedAtDmo = true;
        if (event.normalized_type === "DISPATCH_CITY_TO_CITY")
            state.hasDispatchCityToCity = true;
        if (event.normalized_type === "RECEIVED_AT_DELIVERY_OFFICE")
            state.hasReceivedAtDeliveryOffice = true;
        if (event.normalized_type === "OUT_FOR_DELIVERY")
            state.hasOutForDelivery = true;
        if (event.normalized_type === "DELIVERED")
        ...
        if (event.normalized_type === "MONEY_ORDER_ISSUED")
            state.hasMosIssued = true;
        if (event.normalized_type === "MOS_BOOKED")
            state.hasMosBooked = true;
        if (event.normalized_type === "MOS_DISPATCH")
            state.hasMosDispatch = true;
        if (event.normalized_type === "MOS_DELIVERED")
        ...
        if (isReturnCompletionHint && event.normalized_type === "DELIVERED") {
            const strictReturnComplete = t.includes("delivered to sender") ||
...
function determineCycleInterpretation(input) {
    ...
    if ((state.hasBooked || state.hasReceivedAtDmo || state.hasDispatchCityToCity)
    ...
    if ((state.hasReceivedAtDeliveryOffice || state.hasOutForDelivery)
        && !state.hasDelivered
        && !state.hasReturnDispatch) {
        return {
            final_status: "PENDING",
 ...
 */
function extractMosHintFromRaw(raw, trackingNumber) {
    const empty = { mosNumber: null, mosDetected: false, isMosRedirect: false, mosDeliveredFromRaw: false };
    if (!raw || typeof raw !== "object")
        return empty;
    const r = raw;
    const tn = trackingNumber.toUpperCase();
    // Helper to get nested tracking object (stored under 'tracking' key in rawJson)
    const nested = (r.tracking && typeof r.tracking === "object" ? r.tracking : r);
    ...
    const isMosRedirect = Boolean(srcTn) && (srcTn.startsWith("VPL") || srcTn.startsWith("COD") || srcTn.startsWith("VPP"));
    ...
    let fromPageText = null;
    const pageText = text(r.page_text || nested.page_text);
    if (pageText) {
        const m = pageText.match(/\b(MOS[A-Z0-9]{4,})\b/i);
        if (m) {
            const candidate = m[1].toUpperCase();
            if (candidate !== tn)
                fromPageText = candidate;
        }
    }
    ...
    if (isMosRedirect || isMosSelected || hasMosDeliveryPhrase) {
        ...
        if (latestEvent) {
            const lastDesc = lower(latestEvent.description);
...
export function interpretTrackingCycles(input) {
    ...
    if (codScope) {
        ...
        if (mosHint.isMosRedirect) {
            ...
            state.hasDelivered = true;
            state.hasMosIssued = true;
            if (!flags.includes("MOS_DETECTED_FROM_RAW"))
                flags.push("MOS_DETECTED_FROM_RAW");
        }
    }
    ...
    if (derived.cycle_status !== "COMPLETED") {
        ...
        const isMosInProgress = derived.cycle_detected === "Cycle 3" && (derived.mos_status === "IN_PROGRESS" || derived.mos_status === "COMPLETED");
        if (!isMosInProgress && essentialForCycle1.some((ok) => !ok))
            flags.push("PARTIAL_SEQUENCE_ACCEPTED");
    }
