...
import { moneyOrderHtml, renderLabelDocumentHtml } from "./templates/labels";
...
import { buildMoneyOrderNumber, moneyOrderBreakdown, normalizeTrackingId, parseIdentifierSequence, shouldApplyPakistanPostValuePayableRules, validateMoneyOrderNumber, } from "./validation/trackingId";
import { pythonSubmitComplaint, pythonTrackBulk, PythonServiceTimeoutError, PythonServiceUnavailableError, } from "./services/trackingService";
...
async function ensureMoneyOrderTables() {
  ...
  `);
    const columns = await prisma.$queryRawUnsafe("PRAGMA table_info(money_orders)");
    const hasColumn = (name) => columns.some((col) => String(col.name).toLowerCase() === name.toLowerCase());
    ...
    await prisma.$executeRawUnsafe("CREATE UNIQUE INDEX IF NOT EXISTS idx_money_orders_mo_number ON money_orders(mo_number)");
    await prisma.$executeRawUnsafe("DROP INDEX IF EXISTS idx_money_orders_user_tracking");
    await prisma.$executeRawUnsafe("CREATE UNIQUE INDEX IF NOT EXISTS idx_money_orders_user_tracking_segment ON money_orders(user_id, tracking_number, segment_index)");
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS idx_money_orders_user_tracking_id ON money_orders(user_id, tracking_id)");
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS idx_money_orders_user_tracking ON money_orders(user_id, tracking_number)");
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS idx_money_orders_issue_date ON money_orders(issue_date)");
...
function hasMoneyOrderAmount(order) {
    return normalizeAmount(order.CollectAmount ?? order.amount) > 0;
}
function resolveOrderShipmentType(order, fallback) {
    const normalized = String(order.shipmentType ?? order.shipmenttype ?? fallback ?? "").trim().toUpperCase();
    return normalized || null;
}
async function allocateNextMoneyOrderNumber(issueDate, reservedNumbers) {
    await ensureMoneyOrderTables();
    let sequence = 1;
    const latest = await prisma.$queryRawUnsafe(`SELECT mo_number FROM money_orders WHERE mo_number LIKE ? ORDER BY LENGTH(mo_number) DESC, mo_number DESC LIMIT 1`, `${buildMoneyOrderNumber(1, issueDate).slice(0, 7)}%`);
...
async function ensureSystemMoneyOrders(userId, rows) {
    const uniqueRows = Array.from(new Map(rows
        ...
        .map((row) => [row.trackingNumber, row])).values());
    ...
    for (const row of uniqueRows) {
        ...
        const existing = await prisma.$queryRawUnsafe(`SELECT seq, tracking_number, tracking_id, mo_number, issue_date, amount, segment_index
       FROM money_orders
       WHERE user_id = ? AND tracking_number = ?
       ORDER BY segment_index ASC, seq ASC`, userId, row.trackingNumber);
        ...
        if (desiredLines.length === 0) {
            if (existing.length > 0) {
                await prisma.$executeRawUnsafe(`DELETE FROM money_orders WHERE user_id = ? AND tracking_number = ?`, userId, row.trackingNumber);
            }
            continue;
        }
        for (const desiredLine of desiredLines) {
            ...
            if (currentRow) {
                await prisma.$executeRawUnsafe(`UPDATE money_orders
           SET mo_number = ?,
               segment_index = ?,
               tracking_id = ?,
               issue_date = ?,
           ...
           WHERE seq = ?`, moNumber, desiredLine.segmentIndex, row.trackingNumber, row.issueDate, desiredLine.moAmount, currentRow.seq);
                continue;
            }
            await prisma.$executeRawUnsafe(`INSERT INTO money_orders (id, user_id, tracking_number, mo_number, segment_index, tracking_id, issue_date, amount)
        ...
        if (staleRows.length > 0) {
            ...
            await prisma.$executeRawUnsafe(`DELETE FROM money_orders WHERE seq IN (${placeholders})`, ...staleRows.map((item) => item.seq));
        }
    }
}
async function getMoneyOrdersByTracking(userId, trackingNumbers) {
    ...
    const rows = await prisma.$queryRawUnsafe(`SELECT seq, tracking_number, tracking_id, mo_number, issue_date, amount, segment_index
     FROM money_orders
     WHERE user_id = ? AND tracking_number IN (${placeholders})
     ORDER BY tracking_number ASC, segment_index ASC, seq ASC`, userId, ...uniqueTracking);
...
const worker = new Worker(labelQueueName, async (bullJob) => {
    const { jobId, generateLabels, autoGenerateTracking, generateMoneyOrder, barcodeMode, printMode, trackingScheme, trackAfterGenerate, carrierType, shipmentType, } = bullJob.data;
    ...
    try {
        ...
        if (job.user) {
            ...
            const hasFullProfileShipper = Boolean(profileShipper.shipperName &&
        ...
        if (!useProfileShipper) {
            const uploadSourceErrors = [];
            orders.forEach((order, idx) => {
                const hasUploadShipper = String(order.shipperName ?? "").trim() &&
                    String(order.shipperPhone ?? "").trim() &&
                    String(order.shipperAddress ?? "").trim() &&
                    String(order.shipperEmail ?? "").trim() &&
                    String(order.senderCity ?? "").trim();
                if (!hasUploadShipper) {
                    uploadSourceErrors.push(`Row ${idx + 2}: upload shipper source must include shipperName, shipperPhone, shipperAddress, shipperEmail, senderCity.`);
                }
            });
        ...
        let moneyOrdersByTracking = new Map();
        if (moneyOrderEligibleOrders.length > 0) {
            const today = new Date().toISOString().slice(0, 10);
            await ensureSystemMoneyOrders(job.userId, moneyOrderEligibleOrders.map((order) => ({
                trackingNumber: String(order.trackingNumber ?? "").trim(),
                amount: order.CollectAmount,
                issueDate: today,
                shipmentType: resolveOrderShipmentType(order, shipmentType),
            })));
            moneyOrdersByTracking = await getMoneyOrdersByTracking(job.userId, moneyOrderEligibleOrders.map((order) => String(order.trackingNumber ?? "").trim()));
        ...
        if (trackAfterGenerate === true) {
            const orderByTracking = new Map(labelOrdersForRender
                .map((o) => [String(o.trackingNumber ?? "").trim(), o])
                .filter(([t]) => Boolean(t)));
            const trackingNumbers = Array.from(new Set(labelOrdersForRender
                .map((o) => String(o.trackingNumber ?? "").trim())
                .filter(Boolean)));
            if (trackingNumbers.length > 0) {
                try {
                    await prisma.$transaction(trackingNumbers.map((trackingNumber) => prisma.shipment.upsert({
                        ...
                        create: {
                            ...
                            rawJson: (() => {
                                ...
                                if (!o)
                                    return null;
                                const collectAmount = o.CollectAmount ?? 0;
                                return JSON.stringify({
                                    TrackingID: trackingNumber,
                                    shipperName: String(o.shipperName ?? "").trim(),
                                    shipperPhone: String(o.shipperPhone ?? "").trim(),
                                    shipperAddress: String(o.shipperAddress ?? "").trim(),
                                    shipperEmail: String(o.shipperEmail ?? "").trim(),
                                    senderCity: String(o.senderCity ?? "").trim(),
                                    consigneeName: String(o.consigneeName ?? "").trim(),
                                    consigneeEmail: String(o.consigneeEmail ?? "").trim(),
                                    consigneePhone: String(o.consigneePhone ?? "").trim(),
                                    consigneeAddress: String(o.consigneeAddress ?? "").trim(),
                                    receiverCity: String(o.receiverCity ?? "").trim(),
                                    CollectAmount: String(collectAmount ?? "0").trim() || "0",
                                    ordered: String(o.ordered ?? "").trim(),
                                    ProductDescription: String(o.ProductDescription ?? "").trim(),
                                    Weight: String(o.Weight ?? "").trim(),
                                    shipmenttype: String(o.shipmenttype ?? "").trim(),
                                    numberOfPieces: String(o.numberOfPieces ?? "").trim(),
                                    tracking: null,
                                });
                            })(),
                        },
                    ...
                    await trackingQueue.add("track-bulk", { jobId, kind: "BULK_TRACK", trackingNumbers }, { jobId });
...
const trackingWorker = new Worker(trackingQueueName, async (bullJob) => {
    ...
    try {
        if (data.kind === "BULK_TRACK") {
            ...
            const resultIndexByTracking = new Map(uniqueTrackingNumbers.map((trackingNumber, index) => [trackingNumber, index]));
            const results = uniqueTrackingNumbers.map((tracking_number) => ({
                tracking_number,
                status: "-",
                city: null,
                latest_date: null,
        ...
        const errorMessage = e instanceof PythonServiceUnavailableError || e instanceof PythonServiceTimeoutError
            ? e.message
            : e instanceof Error
                ? e.message
                : "Tracking job failed";
