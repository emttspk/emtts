...
const allowedTrackingPrefixes = [TRACKING_PREFIX];
...
export function normalizeMoneyOrderNumber(value) {
    return String(value ?? "").trim().toUpperCase().replace(/\s+/g, "");
}
export function normalizeShipmentType(value) {
    return String(value ?? "").trim().toUpperCase();
}
export function normalizeCarrierType(value) {
    return String(value ?? "").trim().toLowerCase() === "courier" ? "courier" : "pakistan_post";
}
export function isPakistanPostCarrier(value) {
    return normalizeCarrierType(value) === "pakistan_post";
}
export function isValuePayableShipmentType(value) {
    const normalized = normalizeShipmentType(value);
    return normalized === "VPL" || normalized === "VPP";
}
export function isMoneyOrderEligibleShipmentType(value) {
    const normalized = normalizeShipmentType(value);
    return normalized === "VPL" || normalized === "VPP" || normalized === "COD";
}
export function shouldChargeMoneyOrderCommission(value) {
    return isValuePayableShipmentType(value);
}
export function shouldShowValuePayableAmount(value) {
    return isMoneyOrderEligibleShipmentType(value);
}
export function shouldApplyPakistanPostValuePayableRules(carrierType, shipmentType) {
    return isPakistanPostCarrier(carrierType) && isMoneyOrderEligibleShipmentType(shipmentType);
}
export function formatIdentifierDateCode(value) {
    const date = value instanceof Date ? value : value ? new Date(value) : new Date();
    const year = String(date.getFullYear()).slice(-2);
    const month = String(date.getMonth() + 1).padStart(2, "0");
    return `${year}${month}`;
}
export function formatIdentifierSequence(sequence) {
...
export function validateMoneyOrderNumber(value) {
...
function commissionFor(blockTotal) {
...
export function moneyOrderBreakdown(total, shipmentType) {
