...
import { buildMoneyOrderNumber, buildTrackingId, moneyOrderBreakdown, shouldApplyPakistanPostValuePayableRules, shouldShowValuePayableAmount, } from "../validation/trackingId.js";
...
function deriveNetCommissionFromGross(grossAmount, shipmentType) {
...
function formatWeightInGrams(value) {
...
function resolveDispatchDate(value) {
...
function resolveMoneyOrderSenderFields(order) {
...
function resolveTracking(o, autoGenerateTracking) {
...
function compactInlineParts(parts) {
    return parts
        .map((value) => String(value ?? "").replace(/\s+/g, " ").trim())
        .filter(Boolean);
}
function getLabelAmountSummary(order) {
...
function resolveMoneyOrderAmount(order) {
...
function renderBoxAmountBlock(summary) {
    if (!summary.appliesPakistanPostRules) {
        return "";
    }
    const formatRs = (value) => (Number.isInteger(value) ? String(value) : value.toFixed(2));
...
function loadHtmlTemplate(candidates, notFoundMessage) {
...
export function generateLabelBarcodeBase64(text) {
    try {
...
export function labelsHtml(orders, opts) {
    ...
    const renderSingleLabel = (o) => {
        ...
        const orderId = String(o.ordered ?? "").trim();
        ...
        const prefixBadgeText = amountSummary.showCalculation
            ...
            : amountSummary.appliesPakistanPostRules
                ...
                : shipmentLabel;
        const dispatchDateLine = `Dispatch Date: ${resolveDispatchDate(o?.issueDate)}`;
    ...
    for (let i = 0; i < orders.length; i += labelsPerPage) {
        const pageData = orders.slice(i, i + labelsPerPage);
...
export function renderLabelDocumentHtml(orders, opts) {
...
export function flyerHtml(orders, opts) {
    const autoGenerateTracking = opts?.autoGenerateTracking === true;
    const renderFlyerLabel = (o) => {
        const carrier = o.carrierType === "courier" ? "Courier" : "Pakistan Post";
        const shipmentType = resolveOrderShipmentType(o);
        const shipmentLabel = displayShipmentType(shipmentType);
        const amountSummary = getLabelAmountSummary(o);
    ...
    const renderSlot = (o) => (o ? renderFlyerLabel(o) : emptySlot());
    const pages = [];
...
export function envelopeHtml(orders, opts) {
    const autoGenerateTracking = opts?.autoGenerateTracking === true;
    ...
    const renderEnvelopePage = (templateBody, o) => {
        const amountSummary = getLabelAmountSummary(o);
        ...
        const valueMap = {
            ...
            "{dispatch_date}": escapeHtml(`Dispatch Date: ${resolveDispatchDate(o?.issueDate)}`),
...
function splitBenchmarkSheets(htmlBody) {
    ...
    if (positions.length === 1) {
        return [htmlBody.slice(positions[0]).trim(), ""];
    }
...
function replaceNth(input, regex, occurrence, replacer) {
    let seen = 0;
...
function splitMoNumber(baseMo, index) {
    if (index <= 0)
        return baseMo;
...
function generateMoBarcodeBase64(text) {
    try {
        const value = String(text ?? "").trim();
        if (!value || value === "-")
            return "";
...
function expandBenchmarkOrders(orders) {
    const expanded = [];
    const usedMoNumbers = new Set();
    for (const order of orders) {
        const explicitMoNumber = String(order?.mo_number ?? "").trim().toUpperCase();
        const explicitMoAmount = toNum(order?.amountRs ?? order?.amount ?? 0);
        ...
        const moAmount = toNum(order?.CollectAmount ?? order?.amount ?? order?.amountRs ?? 0);
        const lines = moneyOrderBreakdown(moAmount, order?.shipmentType ?? order?.shipmenttype);
        if (lines.length <= 1) {
        ...
        const baseMo = String(order?.mo_number ?? "-").trim().toUpperCase() || "-";
...
function clearBenchmarkSlot(htmlBody, slotIndex) {
    let out = htmlBody;
    const transparent = "data:image/gif;base64,R0lGODlhAQABAAAAACwAAAAAAQABAAA=";
    out = replaceNth(out, /(<img class="barcode" src=")([^"]*)(" alt="MO Barcode" style=")([^"]*)(" \/>)/g, slotIndex, (_m, p1, _src, p3, style, p5) => `${p1}${transparent}${p3}${style};visibility:hidden${p5}`);
    ...
    out = replaceNth(out, /(<div class="field en" style="left:15\.56mm;top:174\.79mm;width:67\.18mm;font-size:1\.83mm;line-height:1\.12;white-space:normal;">)([\s\S]*?)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${p3}`);
    return out;
}
function fillBenchmarkSlot(htmlBody, slotIndex, order) {
    if (!order)
        return clearBenchmarkSlot(htmlBody, slotIndex);
    const moNumber = String(order?.mo_number ?? "-").trim().toUpperCase() || "-";
    const generatedTrackingId = String(order?.barcodeValue ?? "").trim();
    const tracking = String(order?.trackingNumber ?? order?.TrackingID ?? generatedTrackingId).trim() || "-";
    const amountMo = resolveMoneyOrderAmount(order);
    const amountDisplay = `${amountMo.toFixed(2)}`;
    const issueDate = String(order?.issueDate ?? "").trim() || formatIssueDate();
    const dispatchDate = issueDate;
    const amountWords = String(order?.amountWords ?? "").trim() || `${amountToWords(amountMo)} Rupees Only`;
    const consigneeName = String(order?.consigneeName ?? "-").trim() || "-";
    const consigneeAddress = normalizeAddressLines(order?.consigneeAddress ?? "-") || "-";
    const consigneePhone = String(order?.consigneePhone ?? "-").trim() || "-";
    const { senderName: shipperName, senderAddress: shipperAddress, senderPhone: shipperPhone } = resolveMoneyOrderSenderFields(order);
    const moBarcode = String(order?.mo_barcodeBase64 ?? "").trim();
    let out = htmlBody;
    // MO barcode image src
    out = replaceNth(out, /(<img class="barcode" src=")([^"]*)(" alt="MO Barcode" style="[^"]*" \/>)/g, slotIndex, (_m, p1, oldSrc, p3) => `${p1}${escapeHtml(moBarcode || oldSrc)}${p3}`);
    // Text below barcode (MOS)
    out = replaceNth(out, /(<div class="field mono en" style="left:9\.10mm;top:80\.33mm;width:41\.01mm;font-size:3\.28mm;text-align:center;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(moNumber)}${p3}`);
    // MO number field
    out = replaceNth(out, /(<div class="field mono en" style="left:57\.43mm;top:39\.03mm;width:28\.29mm;font-size:3\.73mm;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(moNumber)}${p3}`);
    // Date
    out = replaceNth(out, /(<div class="field urdu" style="left:40\.13mm;top:162\.57mm;width:28\.99mm;font-size:\.16mm;"><span class="en" style="display:inline-block;font-size:3\.25mm;">)([^<]*)(<\/span><\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(issueDate)}${p3}`);
    // Amount inline
    out = replaceNth(out, /(<div class="field urdu" style="left:72\.73mm;top:140\.37mm;width:44\.55mm;font-size:2\.16mm;">\s*<span class="en" style="display:inline-block;font-size:5\.37mm;">)([^<]*)(<\/span><\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(amountDisplay)}${p3}`);
    // Amount box
    out = replaceNth(out, /(<div class="field mono en" style="left:28\.5mm;top:52\.45mm;width:39\.60mm;text-align:center;font-size:8\.53mm;font-weight:900;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(amountDisplay)}${p3}`);
    // VP tracking
    out = replaceNth(out, /(<div class="field urdu" style="left:90\.27mm;top:48\.04mm;width:45\.26mm;font-size:2\.10mm;"><span class="mono en" style="display:inline-block;font-size:4\.28mm;">)([^<]*)(<\/span><\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(tracking)}${p3}`);
    // Amount words
    out = replaceNth(out, /(<div class="field regular en" style="left:91\.69mm;top:55\.33mm;width:43\.84mm;font-size:2\.89mm;white-space:normal;line-height:2\.06;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(amountWords)}${p3}`);
    // Receiver fields
    out = replaceNth(out, /(<div class="field strong en" style="left:14\.56mm;top:93\.39mm;width:65\.06mm;font-size:2\.58mm;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(consigneeName)}${p3}`);
    out = replaceNth(out, /(<div class="field regular en" style="left:14\.56mm;top:96\.86mm;width:65\.06mm;font-size:2\.13mm;white-space:normal;line-height:1\.06;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(consigneeAddress)}${p3}`);
    out = replaceNth(out, /(<div class="field mono en" style="left:97\.56mm;top:100\.27mm;width:65\.06mm;font-size:2\.13mm;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(consigneePhone)}${p3}`);
    // Sender fields
    out = replaceNth(out, /(<div class="field strong en" style="left:47\.56mm;top:105\.69mm;width:65\.06mm;font-size:4\.(?:58|95)mm(?:;line-height:1\.08)?;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(shipperName)}${p3}`);
    out = replaceNth(out, /(<div class="field regular en" style="left:[0-9.]+mm;top:112\.15mm;width:65\.06mm;font-size:3\.(?:13|35)mm;white-space:normal;line-height:1\.(?:06|12);">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(shipperAddress)}${p3}`);
    out = replaceNth(out, /(<div class="field mono en" style="left:82\.56mm;top:116\.57mm;width:65\.06mm;font-size:4\.(?:13|35)mm(?:;line-height:1\.06)?;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(shipperPhone)}${p3}`);
    // Bottom summary block (receiver + MOS + amount)
    out = replaceNth(out, /(<div class="field en" style="left:15\.56mm;top:174\.79mm;width:67\.18mm;font-size:1\.83mm;line-height:1\.12;white-space:normal;">)([\s\S]*?)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}\n      ${escapeHtml(consigneeName)} | ${escapeHtml(consigneePhone)}<br/>\n      ${escapeHtml(consigneeAddress)}<br/>\n      MOS: ${escapeHtml(moNumber)} | ${escapeHtml(amountDisplay)}\n    ${p3}`);
    // Bottom tracking line
    out = replaceNth(out, /(<div class="field mono en" style="left:15\.56mm;top:198\.83mm;width:63\.64mm;font-size:2\.22mm;">)([^<]*)(<\/div>)/g, slotIndex, (_m, p1, _old, p3) => `${p1}${escapeHtml(tracking)}${p3}` +
...
function moneyOrderHtmlFromBenchmark(orders) {
    ...
    const headWithPrintGuard = head.replace(/<\/head>/i, "<style>body{font-size:0;line-height:0}.sheet{font-size:0;line-height:0}.page{page-break-after:always}.page:last-child{page-break-after:auto}</style></head>");
...
function frontFields(o) {
    const moNumber = String(o.mo_number ?? "").trim().toUpperCase() || "-";
    const generatedTrackingId = String(o.barcodeValue ?? "").trim();
    const tracking = String(o.trackingNumber ?? o.TrackingID ?? generatedTrackingId).trim() || "-";
    const amountRaw = resolveMoneyOrderAmount(o);
    const amountDisplay = Number.isFinite(amountRaw) ? amountRaw.toFixed(2) : "0.00";
    const showMoText = moNumber !== "-";
    const moBarcode = String(o.mo_barcodeBase64 ?? "").trim();
    const issueDate = String(o.issueDate ?? "").trim() || formatIssueDate();
    const amountWords = String(o.amountWords ?? "").trim() || `${amountToWords(amountRaw)} Rupees Only`;
    const consigneeName = String(o.consigneeName ?? "").trim() || "-";
    const consigneeAddress = normalizeAddressLines(o.consigneeAddress ?? "") || "-";
    const consigneePhone = String(o.consigneePhone ?? "").trim() || "-";
