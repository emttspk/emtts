...
export async function parseOrdersFromFile(filePath, opts) {
    ...
    const sourceToStrict = new Map();
    const strictToSource = new Map();
    const headerKeys = Object.keys(firstRow);
    const normalizedHeaderMap = new Map(headerKeys.map(key => [normalizeKey(key), key]));
    if (normalizedHeaderMap.has("bookingcity"))
        normalizedHeaderMap.set("sendercity", normalizedHeaderMap.get("bookingcity"));
    if (normalizedHeaderMap.has("consigneecity"))
        normalizedHeaderMap.set("receivercity", normalizedHeaderMap.get("consigneecity"));
    ...
    const processedData = jsonData.map((row, i) => {
        const strictRow = {};
