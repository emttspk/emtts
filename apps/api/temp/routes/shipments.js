...
async function getMoneyOrderMap(userId, trackingNumbers) {
    await ensureMoneyOrderTables();
    if (trackingNumbers.length === 0)
        return new Map();
    const placeholders = trackingNumbers.map(() => "?").join(",");
    const rows = await prisma.$queryRawUnsafe(`SELECT mo_number, tracking_number, segment_index, amount
     FROM money_orders
     WHERE user_id = ? AND tracking_number IN (${placeholders})
     ORDER BY tracking_number ASC, segment_index ASC, mo_number ASC`, userId, ...trackingNumbers);
    const grouped = new Map();
    rows.forEach((row) => {
        const trackingNumber = String(row.tracking_number ?? "").trim();
        const moNumber = String(row.mo_number ?? "").trim();
        if (!trackingNumber || !moNumber)
            return;
...
function resolveConsigneeFields(raw) {
    const tracking = raw.tracking && typeof raw.tracking === "object"
        ? raw.tracking
        : {};
    return {
        consignee_name: String(raw.receiver_name ??
            raw.receiverName ??
            raw.consignee_name ??
            raw.consigneeName ??
            ...
            "").trim() || null,
        consignee_address: String(raw.receiver_address ??
            raw.receiverAddress ??
            ...
            "").trim() || null,
        consignee_phone: String(raw.receiver_phone ??
            raw.receiverPhone ??
            raw.consignee_phone ??
            raw.consigneePhone ??
...
function parseShipmentEvents(raw) {
...
shipmentsRouter.get("/stats", async (req, res) => {
    ...
    for (const s of shipments) {
        ...
        if (key === "RETURN")
            returnedAmount += amt;
        if (isComplaintEnabled(s.daysPassed) && key !== "DELIVERED")
            delayedAmount += amt;
...
shipmentsRouter.get("/", async (req, res) => {
    ...
    const moMap = await getMoneyOrderMap(userId, shipments.map((s) => s.trackingNumber));
    const enriched = shipments.map((shipment) => {
        ...
        const preferredStatus = String(raw.final_status
            ?? raw.system_status
            ?? raw.System_Status
        ...
        const manualStatus = normalizeManualStatus(raw.manual_status);
        const manualOverrideActive = Boolean(raw.manual_override) && Boolean(manualStatus);
        ...
        const mergedRaw = JSON.stringify({
            ...raw,
            collected_amount: normalizeCollectedAmount(raw?.collected_amount ?? raw?.collect_amount ?? raw?.CollectAmount),
...
shipmentsRouter.post("/cycle-audit/corrections", async (req, res) => {
    ...
    const body = z
        ...
        .parse(req.body ?? {});
    const saved = await saveTrackingCycleCorrections(body.corrections);
    // Persist correction note in shipment rawJson for traceability without schema change.
    await Promise.all(body.corrections.map(async (row) => {
...
shipmentsRouter.patch("/:id", async (req, res) => {
    ...
    const normalizedBody = {
        ...(req.body ?? {}),
        status: String(req.body?.status ?? "").trim().toUpperCase() === "RETURNED"
            ? "RETURN"
            : req.body?.status,
    };
    const body = z
        ...
        return prisma.shipment.update({
            ...
            data: {
                rawJson: JSON.stringify({
                    ...
                    complaint_eligible: manualPending ? true : raw?.complaint_eligible,
                }),
            },
        });
    }));
...
shipmentsRouter.post("/refresh-pending", async (req, res) => {
    ...
    console.log(`[BulkTracking] Cache pre-filter (DB) | total=${shipments.length} cached=${cachedCount} non_cached=${needsRefresh.length}`);
    const consumeInfoMap = new Map();
    const approvedForFetch = [];
    ...
    for (const shipment of approvedForFetch) {
        ...
        try {
            ...
            const collectedAmount = normalizeCollectedAmount(raw?.collected_amount ??
                raw?.collect_amount ??
                raw?.CollectAmount ??
                trackedRaw?.collected_amount ??
                trackedRaw?.collect_amount ??
                trackedRaw?.CollectAmount);
            const processed = processTracking({ ...raw, tracking: trackedRaw, collected_amount: collectedAmount }, { explicitMo: explicitSystemMo, trackingNumber });
            ...
            const mergedRaw = {
                ...
                tracking: trackedRaw,
                tracking_patch: tracked?.meta ?? undefined,
                collected_amount: collectedAmount,
                booking_office: tracked.raw?.booking_office ?? raw?.booking_office,
                delivery_office: tracked.raw?.delivery_office ?? raw?.delivery_office,
                resolved_delivery_office: processed.resolvedDeliveryOffice,
                trackingCacheAt: now,
