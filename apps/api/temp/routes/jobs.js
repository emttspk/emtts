...
import { previewLabelHtml, renderLabelDocumentHtml } from "../templates/labels";
...
function moneyOrderUnitsForAmount(total) {
...
async function ensureJobDeletionSchedulesTable() {
  ...
  `);
    await prisma.$executeRawUnsafe("CREATE INDEX IF NOT EXISTS idx_job_deletion_schedules_delete_after ON job_deletion_schedules(delete_after_at)");
}
async function removeStoredFile(relPath) {
    if (!relPath)
        return;
    try {
...
async function deleteJobById(userId, jobId) {
...
function resolveMoneyOrderRelPath(jobId, relPath) {
    if (relPath) {
...
async function waitForResolvedMoneyOrderRelPath(jobId, relPath, attempts = 6, delayMs = 250) {
...
function parsePrintMode(value) {
    ...
    if (normalized === "envelope" || normalized === "envelope-9x4")
        return "envelope";
    if (normalized === "flyer")
        return "flyer";
    return "labels";
}
export const labelUploadMiddleware = (req, res, next) => {
...
export const labelPreviewUploadMiddleware = (req, res, next) => {
...
jobsRouter.post("/preview/labels", requireAuth, labelPreviewUploadMiddleware, async (req, res) => {
    ...
    const shipmentType = shipmentTypeRaw === "RL" || shipmentTypeRaw === "UMS" || shipmentTypeRaw === "VPL" || shipmentTypeRaw === "VPP" || shipmentTypeRaw === "PAR" || shipmentTypeRaw === "COD" || shipmentTypeRaw === "COURIER"
        ? shipmentTypeRaw
        : null;
...
jobsRouter.post("/delete", requireAuth, async (req, res) => {
    ...
    const jobIds = Array.isArray(req.body?.jobIds)
        ? req.body.jobIds.map((value) => String(value ?? "").trim()).filter(Boolean)
        : [];
    ...
    if (deleteAfterDays === 7) {
        ...
        for (const jobId of jobIds) {
            await prisma.$executeRawUnsafe(`INSERT OR REPLACE INTO job_deletion_schedules (job_id, user_id, delete_after_at) VALUES (?, ?, ?)`, jobId, userId, deleteAfterAt);
        }
...
export async function handleLabelUpload(req, res) {
    const userId = req.user.id;
    await ensureStorageDirs();
    if (!req.file)
        return res.status(400).json({ success: false, error: "Missing file", message: "Missing file" });
    ...
    try {
        ...
        if (ordersCount > 5000)
            throw new Error("Max upload size is 5000 records");
        ...
        if (effectiveGenerateMoneyOrder) {
            const hasMoneyOrderAmount = orders.some((order) => toNum(order?.CollectAmount ?? order?.amount ?? 0) > 0);
        ...
        if (effectiveGenerateMoneyOrder) {
            ...
            for (let i = 0; i < orders.length; i += 1) {
                const amount = toNum(orders[i]?.CollectAmount ?? orders[i]?.amount ?? 0);
    ...
    try {
        ...
        await labelQueue.add("generate-pdf", {
            jobId: job.id,
            generateLabels: true,
...
jobsRouter.get("/:jobId/download/labels", requireAuth, async (req, res) => {
    ...
    if (owned.status !== "COMPLETED")
        return res.status(409).json({ success: false, message: "Not ready" });
    // Prefer BullMQ return value, fall back to DB path
    let relPath = owned.labelsPdfPath;
    try {
        const bullJob = await labelQueue.getJob(jobId);
        if (bullJob?.finishedOn) {
            const result = (await bullJob.returnvalue);
            if (result?.labelsPath)
                relPath = result.labelsPath;
        }
    }
    catch {
...
jobsRouter.get("/:jobId/download/money-orders", requireAuth, async (req, res) => {
    ...
    if (!owned)
        return res.status(404).json({ success: false, message: "Not found" });
    if (!owned.includeMoneyOrders)
        return res.status(404).json({ success: false, message: "Money orders not enabled for this job" });
    if (owned.status !== "COMPLETED") {
        return res.status(409).json({ success: false, status: "processing", message: "Money order is processing" });
    }
    // Prefer BullMQ return value, fall back to DB path
    let relPath = owned.moneyOrderPdfPath;
    try {
        const bullJob = await labelQueue.getJob(jobId);
        if (bullJob?.finishedOn) {
            const result = (await bullJob.returnvalue);
            if (result?.moneyOrderPath)
                relPath = result.moneyOrderPath;
        }
    }
    catch {
        // Redis unavailable — use DB path
