...
async function cleanupScheduledJobDeletions() {
    ...
    const rows = await prisma.$queryRawUnsafe(`SELECT job_id, user_id, delete_after_at FROM job_deletion_schedules`);
    for (const row of rows) {
        if (new Date(row.delete_after_at).getTime() > Date.now())
            continue;
