...
export function moneyOrderHtml(
  ...
  const cards = orders.map((o, index) => {
    ...
    const tracking = String((o as any).trackingNumber ?? (o as any).TrackingID ?? "-").trim() || "-";
    const amount = resolveMoneyOrderAmount(o as any);
    ...
    const receiverName = String((o as any).consigneeName ?? "-").trim() || "-";
    const receiverAddress = normalizeAddressLines((o as any).consigneeAddress ?? "-") || "-";
    const receiverPhone = String((o as any).consigneePhone ?? "-").trim() || "-";
