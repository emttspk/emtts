...
export default function AuthShell(props: AuthShellProps) {
  ...
  return (
    <div className="relative min-h-screen min-h-[100svh] overflow-hidden bg-[linear-gradient(180deg,#f5fbff_0%,#edf6ff_52%,#eefbf6_100%)] px-3 py-2 font-auth sm:px-4 lg:px-5 lg:py-3">
      ...
      <motion.div
        aria-hidden="true"
        className="pointer-events-none absolute right-0 top-0 h-72 w-72 rounded-full bg-[#0ea576]/14 blur-3xl"
        animate={{ x: [0, -22, 0], y: [0, 18, 0] }}
        transition={{ duration: 16, repeat: Number.POSITIVE_INFINITY, ease: "easeInOut" }}
      />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(47,126,219,0.12),transparent_26%),radial-gradient(circle_at_top_right,rgba(14,165,118,0.09),transparent_18%),linear-gradient(rgba(10,31,68,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(10,31,68,0.03)_1px,transparent_1px)] bg-[size:auto,auto,22px_22px,22px_22px] [mask-image:linear-gradient(180deg,rgba(255,255,255,0.7),rgba(255,255,255,0.2))]" />

      <motion.div
        className="relative mx-auto w-full max-w-[1360px] overflow-hidden rounded-[28px] border border-white/80 bg-white/72 shadow-[0_24px_90px_rgba(10,31,68,0.14)] backdrop-blur-2xl"
      ...
      >
        <div className="grid min-h-[calc(100vh-1rem)] min-h-[calc(100svh-1rem)] md:grid-cols-[0.94fr_0.96fr] xl:grid-cols-[1fr_1fr]">
          <section className="relative flex flex-col overflow-hidden border-b border-white/60 bg-[linear-gradient(160deg,rgba(255,255,255,0.92),rgba(237,247,255,0.84))] p-4 sm:p-5 md:border-b-0 md:border-r md:border-r-white/60 md:p-5 lg:p-5 xl:p-6">
            ...
            >
              <span className="inline-flex items-center gap-2 rounded-full border border-[#0ea576]/20 bg-white/80 px-3 py-1.5 text-[0.68rem] font-semibold uppercase tracking-[0.2em] text-[#0b7f6d] shadow-[0_10px_24px_rgba(14,165,118,0.1)]">
                Secure Access
                <ArrowRight className="h-3.5 w-3.5" />
              </span>
              <h1 className="mt-2.5 text-[2rem] font-extrabold tracking-[-0.05em] text-[#0f1f3a] sm:text-[2.25rem] lg:text-[2.55rem] lg:leading-[0.98] xl:text-[2.9rem]">
                ePost.pk
              </h1>
            ...
            <div className="relative z-10 mt-3 grid gap-2 sm:grid-cols-3 md:mt-3 lg:mt-auto">
              {featureItems.map(({ icon: Icon, title }, index) => (
                <motion.div
                  key={title}
                  className="rounded-[20px] border border-white/70 bg-white/72 p-3 shadow-[0_12px_30px_rgba(10,31,68,0.08)] backdrop-blur-xl"
                  initial={{ opacity: 0, y: 16 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.45, delay: 0.22 + index * 0.08, ease: "easeOut" }}
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-[linear-gradient(135deg,rgba(47,126,219,0.16),rgba(14,165,118,0.2))] text-[#0b7f6d] shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
          ...
          <section className="relative flex items-center justify-center bg-[linear-gradient(180deg,rgba(255,255,255,0.54),rgba(246,252,255,0.78))] px-4 py-4 sm:px-5 sm:py-5 md:px-6 lg:px-7 xl:px-8">
            <div className="w-full max-w-[480px]">
              ...
              >
                <h1 className="text-[1.7rem] font-extrabold tracking-[-0.05em] text-[#0f1f3a] sm:text-[1.8rem]">{props.title}</h1>
                {props.subtitle ? <p className="mt-1.5 text-[14px] leading-5 text-[#64748B]">{props.subtitle}</p> : null}
                <div className="mt-4">{props.children}</div>
              </motion.div>

              <motion.div
                className="mt-3 grid gap-2 rounded-[22px] border border-white/75 bg-white/55 p-2.5 shadow-[0_20px_50px_rgba(10,31,68,0.08)] backdrop-blur-xl sm:grid-cols-2"
                initial={{ opacity: 0, y: 18 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                {trustItems.map(({ icon: Icon, label }) => (
                  <div key={label} className="flex items-center gap-2 rounded-2xl px-1 py-0.5">
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-2xl bg-[#0ea576]/12 text-[#0b7f6d]">
                      <Icon className="h-4.5 w-4.5" strokeWidth={2.1} />
                    </div>
