...
export default function PostageSummaryCard({ summary }: { summary: Summary }) {
  return (
    <Card className="border-slate-200 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-xs text-slate-500">Quote only. No service charges, pickup charges, or booking confirmation in Phase 1.</p>
      <div className="mt-3 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="rounded-xl border border-violet-200 bg-violet-50 px-3 py-2">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-violet-800">Total Postage</div>
          <div className="mt-0.5 text-base font-bold text-violet-900">Rs. {summary.totalPostageAmount}</div>
        </div>
