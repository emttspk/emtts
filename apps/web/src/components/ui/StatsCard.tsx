...
export default function StatsCard({ title, value, detail, icon: Icon, tone = "blue", className, onClick }: StatsCardProps) {
  ...
  return (
    <Comp
      ...
      onClick={onClick}
      className={cn(
        "w-full rounded-[18px] border border-[color:var(--line)] bg-[linear-gradient(135deg,var(--tw-gradient-stops))] p-3 text-left shadow-[0_12px_30px_rgba(8,18,37,0.06)] transition-all duration-200 hover:-translate-y-0.5",
        toneClass.shell,
        toneClass.ring,
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[color:var(--text-muted)]">{title}</div>
          <div className="mt-1.5 text-[1.25rem] font-extrabold tracking-[-0.04em] text-[color:var(--text-strong)]">{value}</div>
          {detail ? <div className="mt-0.5 text-[11px] leading-[16px] text-[color:var(--text-muted)]">{detail}</div> : null}
        </div>
        <div className={cn("flex h-8.5 w-8.5 items-center justify-center rounded-2xl shadow-[inset_0_1px_0_rgba(255,255,255,0.6)]", toneClass.icon)}>
