...
const styles: Record<JobStatus, { label: string; cls: string }> = {
  QUEUED: { label: "Queued", cls: "bg-gray-100 text-gray-700 ring-gray-200" },
...
export default function StatusBadge(props: { status: JobStatus }) {
  ...
  return (
    <span className={cn("inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ring-1 ring-inset", s.cls)}>
      {s.label}
    </span>
  );
}
